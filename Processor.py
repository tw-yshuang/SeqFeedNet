...
class Processor:
    def __init__(
        ...
        self.best_record_names = ['stableF_score', *ORDER_NAMES]

        self.console = Console()
        self.epoch = 0
        self.best_epoch = 0
        self.best_records = torch.zeros(len(self.best_record_names), dtype=torch.float32) * 0
        ...
        **kwargs,
        ...
        for self.epoch in range(num_epochs):
            ...
            for i, (data_info, tasking, name) in enumerate(zip(data_infos, [self.__validating, self.__testing], ['Val', 'Test'])):
                ...
                # the order is follow `self.best_record_names`
                last_scores = torch.tensor(
                    [self.summaries[-1].overall.last_scores[BasicRecord.f1_idx], *self.summaries[-1].overall.last_scores]
                )

                best_loss_checker = best_loss_records > last_scores[self.loss_idx :]
                best_loss_records[best_loss_checker] = last_scores[self.loss_idx :][best_loss_checker]

                best_acc_checker = best_acc_record < last_scores[: self.loss_idx]
                if best_acc_checker[0] and not self.summaries[-1].overall.isF1Stable:
                    best_acc_checker[0] = False
                best_acc_record[best_acc_checker] = last_scores[: self.loss_idx][best_acc_checker]
            ...
            save_path = f'loss-{self.summaries[-1].overall.last_scores[self.loss_idx]:.3e}_F1-{self.summaries[-1].overall.last_scores[BasicRecord.f1_idx]:.3f}'
            if isCheckpoint:
            ...
            if isBest:
                ...
                save_path_heads.extend(
                    [f'bestAcc-{name}' for name, is_best in zip(self.best_record_names, best_acc_checker) if is_best],
                )
...
def execute(parser: Parser):
    ...
    if '.FG' in parser.OUT:
        dataset_cfg.frame_groups = int(parser.OUT[parser.OUT.find('.FG') + 3 :].split('.')[0])
    if '.GAP' in parser.OUT:
        dataset_cfg.gap_range[-1] = int(parser.OUT[parser.OUT.find('.GAP') + 4 :].split('.')[0])
        ...
        [
            transforms.RandomChoice(
                [
                    ...
                    PTZZoomCrop(
                        parser.SIZE_HW,
                        min_orig_size=(112, 112),
                        empty_move_time=10,
                        recent_move_time=5,
                        p4targets=0.75,
                        p4others=0.9,
                        p4extraStaticObj=0.75,
                        extraStaticObj_CV_SET=parser.CV_SET,
                        datasetCfg=dataset_cfg,
                    ),
                    PTZPanCrop(
                        parser.SIZE_HW,
                        empty_move_time=10,
                    ...
                    SimilarStaticObjPaste(
                        parser.SIZE_HW,
                        coverMethod='mix',
                        duplicatedObj_range=(1, 7),
                        extraStaticObj_CV_SET=parser.CV_SET,
                        datasetCfg=dataset_cfg,
                    ),
                ],
                p=(0.15, 0.175, 0.175, 0.35 * 0.4, 0.35 * 0.6, 0.15),
            ),
