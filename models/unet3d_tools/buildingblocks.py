...
from models.unet3d_tools.se import ChannelSELayer3D, ChannelSpatialSELayer3D, SpatialSELayer3D
...
class DoubleConv(nn.Sequential):
    ...
    def __init__(self, in_channels, out_channels, encoder, kernel_size=3, order='gcr', num_groups=8, padding=1, is3d=True):
        ...
        if encoder:
            ...
            # print(f"encoder : {conv1_in_channels} {conv1_out_channels} | {conv2_in_channels} {conv2_out_channels}")
        else:
            # we're in the decoder path, decrease the number of channels in the 1st convolution
            conv1_in_channels, conv1_out_channels = in_channels, out_channels
            conv2_in_channels, conv2_out_channels = out_channels, out_channels
            # print(f"decoder : {conv1_in_channels} {conv1_out_channels} | {conv2_in_channels} {conv2_out_channels}")

        # conv1
        self.add_module(
            'SingleConv1',
            SingleConv(conv1_in_channels, conv1_out_channels, kernel_size, order, num_groups, padding=padding, is3d=is3d),
        )
        # conv2
        self.add_module(
            'SingleConv2',
            SingleConv(conv2_in_channels, conv2_out_channels, kernel_size, order, num_groups, padding=padding, is3d=is3d),
        )
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        if self.pooling is not None:
            # ! modified to use 2D pooling
            # ? Pool spatial information
            pl = self.pooling(x[:, :, 0, :, :])
            S, (B, C, H, W) = x.shape[2], pl.shape  # * S: sequential; B: batch; C: Channel; H: height; W: width
            y = torch.empty((B, C, S, H, W), device=x.device)
            y[:, :, 0, :, :] = pl
            for s in range(S, 1):
                y[:, :, s, :, :] = self.pooling(x[:, :, s, :, :])
            x = y
        x = self.basic_module(x)
        # print('Fin encoder basic module',self.basic_module)
        return x
...
class Decoder(nn.Module):
        ...
        if upsample:
            if basic_module == DoubleConv:
                ...
                self.upsampling = TransposeConvUpsampling(
                    in_channels=in_channels, out_channels=out_channels, kernel_size=conv_kernel_size, scale_factor=scale_factor
                )
                # sum joining
    ...
    def forward(self, encoder_features, x):
        # print('decoder forward: ', encoder_features.shape, x.shape)
...
def create_encoders(
    in_channels, f_maps, basic_module, conv_kernel_size, conv_padding, layer_order, num_groups, pool_kernel_size, is3d
):
