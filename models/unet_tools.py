...
class UNetDown(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, num_rep, batch_norm=False, activation=nn.ReLU(), kernel_size=3, dropout=False, maxpool=False):
        ...
        for k in range(num_rep):
            self.down_block.add_module(
                "conv%d" % (k + 1), nn.Conv2d(in_ch_for_conv, out_ch, kernel_size=kernel_size, padding=(int((kernel_size - 1) / 2)))
            )
...
class UNetUp(nn.Module):
    ...
    def __init__(self, in_ch, res_ch, out_ch, num_rep, batch_norm=False, activation=nn.ReLU(), kernel_size=3, dropout=False):
        ...
        self.up.add_module(
            "conv2d_transpose",
            nn.ConvTranspose2d(
                in_ch, in_ch, kernel_size, stride=2, output_padding=(int((kernel_size - 1) / 2)), padding=(int((kernel_size - 1) / 2))
            ),
        )
        if batch_norm:
