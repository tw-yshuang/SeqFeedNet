...
from models.unet3d_tools.buildingblocks import DoubleConv, ResNetBlock, ResNetBlockSE, create_decoders, create_encoders
...
class AbstractUNet(nn.Module):
    ...
    def __init__(
        ...
        # create encoder path
        # print(in_channels, f_maps, basic_module, conv_kernel_size, conv_padding, layer_order, num_groups , pool_kernel_size, is3d)
        self.encoders = create_encoders(
            in_channels, f_maps, basic_module, conv_kernel_size, conv_padding, layer_order, num_groups, pool_kernel_size, is3d
        )

        # create decoder path
        self.decoders = create_decoders(f_maps, basic_module, conv_kernel_size, conv_padding, layer_order, num_groups, is3d)
        ...
        self.final_3dcmpr = nn.Conv3d(f_maps[0], out_channels, 1)
        # self.final_2dcmpr = nn.Conv2d(3, 1, 1)
    ...
    def forward(self, x):
        ...
        x = self.final_3dcmpr(x)
        # x = x[:, 0, :, :, :]
        # x = self.final_2dcmpr(x)
