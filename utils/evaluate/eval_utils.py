...
class BasicRecord:
    __row_id = 0
    f1_idx = ORDER_NAMES.index('F_score')
    ...
    # the stable F_score model have each epoch less than 0.08 gap 5 times
    @property
    def isF1Stable(self):
        if self.__row_id < 6:
            return False
        else:
            begin_row_id = self.__row_id - 6

            prev = self.__score_records[begin_row_id : self.__row_id - 1, self.f1_idx]
            curr = self.__score_records[begin_row_id + 1 : self.__row_id, self.f1_idx]

            return torch.all(torch.abs(curr - prev) < 0.08)
