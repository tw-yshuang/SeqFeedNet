...
class LASIESTAPreprocess:
    ...
    def __call__(self, gt: np.ndarray) -> np.ndarray:
        ...
        label = np.float32(label if self.image_size == None else cv2.resize(label, dsize=self.image_size, interpolation=cv2.INTER_NEAREST))
...
<<<<<<< HEAD
    ...
    # ! Test for Lasiesta
    data_processes = LASIESTAPreprocess(image_size=(244, 244))  # all pass
